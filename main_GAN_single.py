...
from src.utils import saveplot_12lead, LoadSingleData, useDataset
...
os.makedirs(f'results/{args.domain}/pulse2pulse/{args.site}', exist_ok=True)
_logpth = f'results/{args.domain}/pulse2pulse/{args.site}/logs/'
if not os.path.exists(_logpth): os.mkdir(_logpth)
logging.basicConfig(filename=os.path.join(_logpth, f'pulse2pulse_{site}_{bs}_{lr}_{d}_site4.log'), 
                    level=logging.DEBUG,
...
train_site_data, local_site_data, global_site_data = LoadSingleData(seed, args.site)    
_dl = DataLoader(useDataset(global_site_data['site1']), batch_size=64, shuffle=True)
...
class pulse2pulse_GP(object):
    ...
    def train(self):
        path = f'results/{args.domain}/pulse2pulse/{args.site}/saved/'
        ...
        prev_path = 'results/single/pulse2pulse/ptb/saved/pulse2pulse_site1_0_64_0.0001_50.tar'
        ...
        for epoch in range(first_iter, first_iter+max_epoch):
            ...
            if (epoch + 1) % args.vartime == 0:
                ...
                figpth = f'results/{args.domain}/pulse2pulse/{args.site}/figure/{_site}/{_flag}'
...
if __name__ == "__main__":    
    try:
        ...
        for _n, (_site, data) in enumerate(train_site_data.items()):
            for flag in flag_group:
                if _n == 3:
                    logging.debug(f'{_site} / flag {flag} training..')
