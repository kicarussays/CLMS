...
from src.utils import LoadSingleData, useDataset, sitesum, seedset, aucfloat
...
parser.add_argument('--es', type=int, 
                    help='early stopping option', default=0)
parser.add_argument('--seed', type=int, 
                    help='Random Seed', default=678)
parser.add_argument('--file', type=str, 
                    help='Data select', default='ptb')
...
parser.add_argument('--domain', type=str, 
                    help='Single or Multiple Domain', default='single')
...
def main():
    ...
    savepath = f'results/{args.domain}/{args.mode}/{args.file}/saved/{params}/'
    logpath = f'results/{args.domain}/{args.mode}/{args.file}/logs/'
    ...
    train_site_data, local_site_data, global_site_data = LoadSingleData(seed, args.file)    
    ...
    if args.mode == 'Supervised':
        ...
        #     # Each site training
        #     model = ResNet1d()
        #     savepath = f'results/{args.domain}/{args.mode}/{args.file}/saved/{params}/{g}/'
        #     os.makedirs(savepath, exist_ok=True)
        ...
        logging.debug(f'\n\nAll training...')
        model = ResNet1d()
        savepath = f'results/{args.domain}/{args.mode}/{args.file}/saved/{params}/all/'
        os.makedirs(savepath, exist_ok=True)
        Trainer = Supervised_Learning(
        ...
        logpth = f'results/single/Supervised/{args.file}/logs/'
        ...
        train_site_data = {f'site{s+1}': train_site_data[f'site{s+1}'] for s in score_rank}
        local_site_data = {f'site{s+1}': local_site_data[f'site{s+1}'] for s in score_rank}
        global_site_data = {f'site{s+1}': global_site_data[f'site{s+1}'] for s in score_rank}
